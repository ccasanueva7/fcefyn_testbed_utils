...
|-------|----------|------------------------------------------------------------------------------------------------------------------------------|
| **Control** | gRPC (port 20408) | Coordinator manages places, reservations, locks. `labgrid-client` and the Labgrid pytest plugin use this. Since Labgrid 25.0 (May 2025); earlier versions used WebSocket/WAMP. |
...
| Component | Role |
|-----------|------|
| `labgrid-coordinator` (port 20408) | gRPC server. Registers places from `places.yaml`, tracks locks and reservations. |
...
| Component | Role |
|-----------|------|
| `labgrid-exporter` | Registers local DUT resources (serial, power, network) with the coordinator over gRPC. |
...
| Register resources | `labgrid-exporter` (lab) | coordinator | gRPC (outbound from lab) |
