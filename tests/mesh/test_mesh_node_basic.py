...
from helpers import node_mac, ssh_run
...
def test_eth2_has_ip(node):
    ...
    assert rc == 0 and expected_prefix in out, f"{node['name']}: eth2 missing expected IP {expected_prefix}: {out}"
...
def test_batman_adv_mesh_active(node):
    ...
    assert rc == 0 and "active" in out, f"{node['name']}: no active batman-adv interface: {out}"
...
def test_uci_vwifi_enabled(node):
    ...
    assert rc == 0 and out.strip() == "1", f"{node['name']}: vwifi not enabled in UCI: '{out}'"


def test_uci_vwifi_server_ip(node):
    rc, out, _ = ssh_run(node["port"], "uci get vwifi.config.server_ip")
    assert rc == 0 and out.strip() == "10.99.0.2", f"{node['name']}: vwifi server IP wrong: '{out}'"
...
def test_uci_wireless_band_2g(node):
    ...
    assert rc == 0 and out.strip() == "2g", f"{node['name']}: radio0 band is '{out}', expected '2g'"


def test_uci_wireless_channel_1(node):
    rc, out, _ = ssh_run(node["port"], "uci get wireless.radio0.channel")
    assert rc == 0 and out.strip() == "1", f"{node['name']}: radio0 channel is '{out}', expected '1'"
